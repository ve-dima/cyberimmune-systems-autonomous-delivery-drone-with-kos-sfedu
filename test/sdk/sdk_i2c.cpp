...
#define I2C_CONFIG "rpi4_bcm2711.p2-3"
...
int startI2C(char* channelName) {
    if (!i2cInit) {
        ...
        rc = BspSetConfig(channelName, I2C_CONFIG);
        if (rc != rcOk) {
...
int writeRegister(uint8_t reg, uint8_t val) {
    ...
    Retcode rc = I2cXfer(i2cH, mpuFrequency, messages, 1);
    if (rc != rcOk) {
...
int readRegister(uint8_t reg, uint8_t& val) {
    ...
    I2cError rc = I2cXfer(i2cH, mpuFrequency, messages, 2);
    if (rc != I2C_EOK) {
...
int readVector(uint8_t reg, Vector3f& val) {
    ...
    val.X = (int16_t)((readBuffer[0] << 8) | readBuffer[1]);
    val.Y = (int16_t)((readBuffer[2] << 8) | readBuffer[3]);
    val.Z = (int16_t)((readBuffer[4] << 8) | readBuffer[5]);
...
float getTemperature() {
    ...
    return ((float)temp / 340.0f + 36.5f);
}